...
public class DeriveDataCopyHandlerImpl implements OperationHandler<MetricMetaDerive, Void> {
    ...
    public String exec(List<MetricMetaDerive> data, Void vo, String[] param) {
        ...
        try {
            ...
            String queryField = String.format("concat('copy_',metric_name),metric_zh_name,metric_type,cal_period,0,     metric_theme,%s,    now(),      product_def,tec_def,upstream_metric,modifier_def,stat_period"
                    , eruptUserService.getCurrentEruptUser().getId());
            String                  insertField    =          "metric_name,metric_zh_name,metric_type,cal_period,status,metric_theme,updater,update_time,product_def,tec_def,upstream_metric,modifier_def,stat_period";
